...
export class MentionsDirective implements OnChanges {
  ...
  // stores the items passed to the mentions directive and used to populate the root items in MentionsConfig
  private mentionItems: any[];
  private activeConfig: MentionsConfig;
  ...
  private triggerChars: { [key: string]: MentionsConfig } = {};
  private searchString: string;
  private startPos: number;
  private startNode;
  private searchList: MentionsListComponent;
  ...
  public updateConfig() {
    const config = this.MentionsConfig;
    this.triggerChars = {};
    ...
    // nested configs
    if (config.mentions) {
      config.mentions.forEach(configuration => this.addConfig(configuration));
    }
  }
  ...
  public startSearch(triggerChar?: string, nativeElement: HTMLInputElement = this._element.nativeElement) {
    triggerChar = triggerChar || this.MentionsConfig.triggerChar || this.DEFAULT_CONFIG.triggerChar;
  ...
  showSearchList(nativeElement: HTMLInputElement) {
    ...
    if (this.searchList == null) {
      const componentFactory = this._componentResolver.resolveComponentFactory(MentionsListComponent);
    ...
    this.searchList.styleOff = this.MentionsConfig.disableStyle;
  ...
  private addConfig(config: MentionsConfig) {
    // defaults
