...
export class SysAdminPageAdsService {
  ...
  getAdById(pageAdId: number): Observable<IPageAds> {
  ...
  createAd(formData): Observable<IPageAds> {
    return this.http.post<IPageAds>(
      this.apiRoutesService.getRoute(API_ROUTES.PAGE_ADS.CREATE), formData
    );
  }

  updateAd(formData, pageAdId: number): Observable<IPageAds> {
    const params = new HttpParams().set('page_ad_id', String(pageAdId));

    return this.http.put<IPageAds>(
