...
export class CommunityEditDetailsComponent implements OnInit {
  ...
  updateCommunityDetails() {
    ...
    Object.keys(communityFormData).forEach((key) =>
      !(communityFormData[key] == null)
        ? formData.append(`community[${key}]`, communityFormData[key])
        : ""
    );
    ...
    this.communitiesService
      .updateCommunity(formData, this.community.id)
      .subscribe((community) => {
        this.toastLogService.successDialog(
          "Updated! Reloading the app for changes to apply..."
        );
