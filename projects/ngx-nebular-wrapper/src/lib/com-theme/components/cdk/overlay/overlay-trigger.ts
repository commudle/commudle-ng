import { ComponentRef, Inject, Injectable, InjectionToken } from '@angular/core';
...
export class NbClickTriggerStrategy extends NbTriggerStrategyBase {
  ...
  protected clickOut$: Observable<Event> = this.clickIn$.pipe(
    switchMap(() =>
      observableFromEvent<Event>(this.document, 'click').pipe(
...
export class NbFocusTriggerStrategy extends NbTriggerStrategyBase {
  ...
  protected tabKeyPress$: Observable<Event> = observableFromEvent<Event>(this.host, 'keydown').pipe(
