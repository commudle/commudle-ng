...
import { ELabPublishStatus, ILab, ILabStep } from '@commudle/shared-models';
import { LibToastLogService, SeoService } from '@commudle/shared-services';
import { NbDialogRef, NbDialogService } from '@nebular/theme';
import { EditorOptions } from "tinymce";
import { LabsService } from '../../services/labs.service';

@Component({
  selector: 'commudle-edit-lab',
...
export class EditLabComponent implements OnInit, OnDestroy {
  ...
  updateLab(publishStatus, forceSubmit: boolean = false) {
    if (this.lab.publish_status !== ELabPublishStatus.published) {
      this.labForm.patchValue({
        publish_status: publishStatus,
      });
    }
    if (this.steps.length < 3 && publishStatus === ELabPublishStatus.submitted && !forceSubmit) {
