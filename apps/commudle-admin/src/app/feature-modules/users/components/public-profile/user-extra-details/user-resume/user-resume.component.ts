...
import { IAttachedFile, ICurrentUser, IUser, IUserResume } from '@commudle/shared-models';
...
export class UserResumeComponent implements OnInit, OnChanges, OnDestroy {
  ...
  onFileChange(event) {
    if (event.target.files && event.target.files.length) {
      ...
      reader.onload = () => (this.uploadedResumeSrc = reader.result as string);
